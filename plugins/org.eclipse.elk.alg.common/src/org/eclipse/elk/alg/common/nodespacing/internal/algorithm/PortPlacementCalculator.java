...
public final class PortPlacementCalculator {
    ...
    /**
     * Copy of the {@link de.cau.cs.kieler.klay.layered.properties.InternalProperties#PORT_RATIO_OR_POSITION} option.
     * For further information see the documentation found there. We added this copy here to allow a generic treatment
     * of spacing calculations for graph elements. See the {@link org.eclipse.elk.alg.common.nodespacing} package.
     * [programmatically set]
     * 
     * TODO This should be replaced by a proper property declared in a MELK file.
     */
    public static final IProperty<Double> PORT_RATIO_OR_POSITION = new Property<Double>("portRatioOrPosition", 0.0);
    ...
    private static void placeHorizontalFreePorts(final NodeContext nodeContext, final PortSide portSide) {
        ...
        double availableSpace =
                insidePortLabelCellRectangle.width - insidePortLabelCellPadding.left - insidePortLabelCellPadding.right;
        ...
        if ((portAlignment == PortAlignment.DISTRIBUTED || portAlignment == PortAlignment.JUSTIFIED)
            ...
            calculatedPortPlacementWidth =
                    modifiedPortPlacementSize(nodeContext, portAlignment, calculatedPortPlacementWidth);
        ...
        for (PortContext portContext : nodeContext.portContexts.get(portSide)) {
            ...
            currentXPos += portContext.portMargin.left + portContext.port.getSize().x + portContext.portMargin.right
                    + spaceBetweenPorts;
        }
    }
    ...
    private static double calculateHorizontalPortYCoordinate(final PortContext portContext) {
        ...
        if (port.hasProperty(CoreOptions.PORT_BORDER_OFFSET)) {
            ...
            return port.getSide() == PortSide.NORTH ? -port.getSize().y : 0;
        }
    }
    ...
    private static void placeVerticalFreePorts(final NodeContext nodeContext, final PortSide portSide) {
        ...
        if ((portAlignment == PortAlignment.DISTRIBUTED || portAlignment == PortAlignment.JUSTIFIED)
            ...
            calculatedPortPlacementHeight =
                    modifiedPortPlacementSize(nodeContext, portAlignment, calculatedPortPlacementHeight);
            portAlignment = PortAlignment.CENTER;
        }
        ...
        for (PortContext portContext : nodeContext.portContexts.get(portSide)) {
            // Alfa
            PortAdapter<?> parentPort = portContext.port.getParentPortAdapter();
            if (parentPort == null) {
                portContext.portPosition.x = calculateVerticalPortXCoordinate(portContext, nodeWidth);
                portContext.portPosition.y = currentYPos + portContext.portMargin.top;

                // Update the y coordinate for the next port
                currentYPos += portContext.portMargin.top + portContext.port.getSize().y + portContext.portMargin.bottom
                        + spaceBetweenPorts;
                // //TODO: need to get the coordinate for each port one by one by calculating it individually
                // setSubPortsPositions(portContext.port);
            } else {
                KVector position = getSubPortsPosition(portContext.port);
                portContext.portPosition.x = position.x;
                portContext.portPosition.y = position.y;
            }
        }
    }
     ...
     */
    private static double modifiedPortPlacementSize(final NodeContext nodeContext, final PortAlignment oldPortAlignment,
            final double currentPortPlacementSize) {
     ...
     * @param elkport
     */
    private static KVector getSubPortsPosition(PortAdapter<?> elkport) {

        KVector position = elkport.getPosition();
        PortAdapter<?> parentPort = elkport.getParentPortAdapter();

        if (parentPort != null && !parentPort.getSubPortAdapters().isEmpty()) {
            PortSide portSide = elkport.getSide();
            double portSize = 5;
            double mid = (((double) parentPort.getSubPortAdapters().size() - 1) / 2d);
            int i = parentPort.getSubPortAdapters().indexOf(elkport);

            if (portSide == PortSide.WEST) {
                double y = elkport.getPosition().y - (portSize * (i - mid));
                elkport.getPosition().y = y;
                elkport.getPosition().x = elkport.getPosition().x - portSize;
                System.out.println(elkport.getPosition());
                ...
                double x = elkport.getPosition().x - (portSize * (i - mid));
                elkport.getPosition().x = x;
                elkport.getPosition().y = elkport.getPosition().y + portSize;
                System.out.println(elkport.getPosition());
            } else if (portSide == PortSide.SOUTH) {
                PortAdapter<?> subport = elkport.getSubPortAdapters().get(i);
                double x = elkport.getPosition().x - (portSize * (i - mid));
                subport.getPosition().x = x;
                subport.getPosition().y = elkport.getPosition().y - portSize;
                System.out.println(subport.getPosition());
            }
     ...
     * @param elkport
     */
    private static void setSubPortsPositions(PortAdapter<?> elkport) {

        if (!elkport.getSubPortAdapters().isEmpty()) {
            //
            PortSide portSide = elkport.getSide();
            double portSize = 5;

            if (portSide == PortSide.WEST) {
                double mid = (((double) elkport.getSubPortAdapters().size() - 1) / 2d);
                for (int i = 0; i < elkport.getSubPortAdapters().size(); i++) {
                    PortAdapter<?> subport = elkport.getSubPortAdapters().get(i);
                    double y = elkport.getPosition().y - (portSize * (i - mid));
                    subport.getPosition().y = y;
                    subport.getPosition().x = elkport.getPosition().x - portSize;
                    System.out.println(subport.getPosition());
                }
            } else if (portSide == PortSide.EAST) {
                double mid = (((double) elkport.getSubPortAdapters().size() - 1) / 2d);

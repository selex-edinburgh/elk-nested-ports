...
class ElkGraphImporter {
            ...
            if (!child.getProperty(LayeredOptions.NO_LAYOUT)) {
                if ((elkgraph.getProperty(LayeredOptions.CONSIDER_MODEL_ORDER_STRATEGY) != OrderingStrategy.NONE
                        || elkgraph.getProperty(
                                LayeredOptions.CYCLE_BREAKING_STRATEGY) == CycleBreakingStrategy.MODEL_ORDER
                        || elkgraph.getProperty(
                                LayeredOptions.CYCLE_BREAKING_STRATEGY) == CycleBreakingStrategy.GREEDY_MODEL_ORDER
                        || elkgraph.getProperty(LayeredOptions.CROSSING_MINIMIZATION_FORCE_NODE_MODEL_ORDER)
                        || elkgraph.getProperty(
                                LayeredOptions.CONSIDER_MODEL_ORDER_COMPONENTS) != ComponentOrderingStrategy.NONE)
    ...
    private LPort transformPort(final ElkPort elkport, final LNode parentLNode,
        ...
        while (side == PortSide.UNDEFINED && parentPort != null) {
            if (parentPort != null) {
                side = parentPort.getProperty(LayeredOptions.PORT_SIDE);
            }
